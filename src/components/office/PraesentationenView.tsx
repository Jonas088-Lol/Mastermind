...
import { createPresentation, createPresentationFromTemplate } from "@/app/app/praesentationen/actions";
import { PRESENTATION_TEMPLATES } from "@/app/app/praesentationen/templates";


export async function PraesentationenView({ basePath }: { basePath: string }) {
    ...
    <div className="mx-auto flex max-w-7xl flex-col gap-8">
      <header className="flex items-end justify-between gap-4">
        <div>
          ...
          <h1 className="text-3xl font-bold tracking-tight sm:text-4xl">MasterSlides</h1>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {presentations.map((pres) => {
            ...
            return (
              <Link
                key={pres.id}
                href={`${basePath}/praesentationen/${pres.id}`}
