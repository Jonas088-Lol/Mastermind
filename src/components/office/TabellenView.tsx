...
import { createSpreadsheet } from "@/app/app/tabellen/actions";


export async function TabellenView({ basePath }: { basePath: string }) {
    ...
    <div className="mx-auto flex max-w-7xl flex-col gap-8">
      <header className="flex items-end justify-between gap-4">
        <div>
          ...
          <h1 className="text-3xl font-bold tracking-tight sm:text-4xl">MasterCalc</h1>
      ...
      {sheets.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {sheets.map((sheet) => (
            <Link
              key={sheet.id}
              href={`${basePath}/tabellen/${sheet.id}`}
