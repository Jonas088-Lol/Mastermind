...
import { createParentThread } from "./actions";
...
export default async function ElternNachrichtNeuPage() {
  const session = await getSession();
  if (!session || effectiveRole(session) !== "parent") redirect("/eltern");
  ...
  return (
    <div className="mx-auto flex max-w-2xl flex-col gap-6">
      <header className="flex items-center gap-3">
        <Link href="/eltern/nachrichten" className="text-muted-fg hover:text-fg">
      ...
      <form action={createParentThread} className="flex flex-col gap-5">
        <div className="flex flex-col gap-1.5">
          ...
          >
            ...
            {Array.from(byRole.entries()).map(([role, users]) => (
        ...
        <div className="flex items-center justify-between gap-4">
          <Link href="/eltern/nachrichten" className="text-sm text-muted-fg hover:text-fg">
            Abbrechen
          </Link>
          <button
            type="submit"
