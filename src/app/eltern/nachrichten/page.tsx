...
export default async function ElternNachrichtenPage() {
    ...
    <div className="mx-auto flex max-w-7xl flex-col gap-6">
      ...
      {participations.length === 0 ? (
        <div className="grid place-items-center border border-dashed border-border p-16 text-center">
          ...
          <p className="mt-1 text-sm text-muted-fg">Kontaktiere Lehrer direkt über diesen Bereich.</p>
        </div>
      ) : (
        <div className="divide-y divide-border border border-border">
          {participations.map((p) => {
            ...
            return (
              <Link
                key={p.thread.id}
                href={`/eltern/nachrichten/${p.thread.id}`}
                className={cn(
