...
export default async function NachrichtenPage() {
  ...
  const unreadCount = participations.filter((p) => {
    ...
    if (last.sender.id === session.userId) return false;
  ...
  return (
    <div className="mx-auto flex max-w-7xl flex-col gap-6">
      <header className="flex items-end justify-between">
        ...
        <Link href="/app/nachrichten/neu" className={buttonVariants({ size: "sm" })}>
      ...
      {participations.length === 0 ? (
        <div className="grid place-items-center border border-dashed border-border p-16 text-center">
          ...
          <p className="mt-4 text-base font-semibold">Noch keine Nachrichten</p>
          <p className="mt-1 text-sm text-muted-fg">Starte eine Konversation mit einem Lehrer oder Elternteil.</p>
        </div>
      ) : (
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className={cn("text-xs text-muted-fg", isUnread && "font-medium text-fg")}>
                    {p.thread.subject}
                  </p>
                  {last && (
                    <p className="mt-0.5 truncate text-xs text-muted-fg">
                      {last.sender.id === session.userId ? "Du: " : ""}{last.content}
                    </p>
                  )}
                </div>
                {isUnread && (
