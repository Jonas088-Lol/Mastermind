...
import { bulkInviteTeachers, type BulkInviteResult } from "./actions";

export function BulkInviteTeacherCard() {
  const [result, setResult] = useState<BulkInviteResult | null>(null);
  const [isPending, startTransition] = useTransition();

  function handleSubmit(formData: FormData) {
    startTransition(async () => {
      const res = await bulkInviteTeachers(formData);
      setResult(res);
    });
  }
      ...
      <div className="flex items-center gap-3 border-b border-border px-5 py-4">
        ...
        <div>
          ...
          <p className="text-xs text-muted-fg">
            Format: <span className="font-mono">Name;E-Mail</span> — eine Zeile pro Lehrkraft
          </p>
        </div>
      </div>
      <div className="p-5">
        <form action={handleSubmit} className="flex flex-col gap-4">
          <textarea
            ...
            placeholder={"Name;E-Mail\nDr. Maria Schmidt;schmidt@schule.de\nTobias Müller;mueller@schule.de"}
          />
