...
async function sendTeacherInvite(
  ...
  await prisma.verificationToken.create({
    data: { email, type: "teacher-invite", token, expiresAt },
  });

  const appUrl = process.env.NEXT_PUBLIC_APP_URL ?? "http://localhost:3000";
  const inviteUrl = `${appUrl}/onboarding?token=${token}&email=${encodeURIComponent(email)}&name=${encodeURIComponent(name)}&role=teacher&schoolId=${schoolId}`;
  ...
  await sendEmail({
    ...
    html: `
      <p>Hallo ${name},</p>
      <p>Du wurdest eingeladen, MasterMind als Lehrkraft an <strong>${schoolName}</strong> zu nutzen.</p>
...
export async function inviteTeacher(formData: FormData): Promise<void> {
  ...
  await sendTeacherInvite(email, name, session.schoolId, school?.name ?? "Deine Schule");

  revalidatePath("/admin/nutzer");
  redirect("/admin/nutzer");
}

export async function bulkInviteTeachers(formData: FormData): Promise<BulkInviteResult> {
  const session = await getSession();
  if (!session || effectiveRole(session) !== "admin") redirect("/login");
  ...
  for (let i = 0; i < dataLines.length; i++) {
    ...
    if (cols.length < 2) {
      errors.push({ line: lineNum, msg: "Zu wenige Spalten (Name;E-Mail)" });
      continue;
    }

    const [name, emailRaw] = cols;
    ...
    const alreadyInvited = await prisma.verificationToken.findFirst({
      where: { email, type: "teacher-invite" },
    });
    if (alreadyInvited) { skipped++; continue; }

    try {
      await sendTeacherInvite(email, name, session.schoolId!, schoolName);
      sent++;
    } catch {
