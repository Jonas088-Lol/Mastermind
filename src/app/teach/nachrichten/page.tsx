...
export default async function TeachNachrichtenPage() {
      ...
      thread: {
        include: {
          ...
          participants: {
            ...
            include: { user: { select: { name: true, role: true, klasse: true } } },
          },
        },
      },
    },
      ...
      {participations.length === 0 ? (
        ...
        <div className="divide-y divide-border border border-border">
          {participations.map((p) => {
            ...
            const roleTag = others[0] ? ROLE_LABEL[others[0].role] : null;
            const klasse = others[0]?.klasse;

            return (
              <Link
                key={p.thread.id}
                href={`/teach/nachrichten/${p.thread.id}`}
                className={cn(
