...
export default async function TeachThreadPage({ params }: Props) {
  const { threadId } = await params;
  ...
  const thread = await prisma.messageThread.findUnique({
    ...
    include: {
      ...
      participants: {
        include: { user: { select: { id: true, name: true, role: true, klasse: true } } },
      },
    },
  });
  if (!thread) notFound();
    ...
    <div className="mx-auto flex max-w-4xl flex-col">
      <div className="sticky top-0 z-10 flex items-center gap-3 border-b border-border bg-bg px-5 py-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-xs text-muted-fg truncate">
            {thread.participants.map((p) => `${p.user.name}${p.user.klasse ? ` (${p.user.klasse})` : ""}`).join(", ")}
          </p>
        </div>
      </div>
      ...
      <div className="sticky bottom-0 border-t border-border bg-bg px-5 py-4">
        <form action={sendMessage} className="flex items-end gap-3">
          ...
          <input type="hidden" name="backHref" value={`/teach/nachrichten/${threadId}`} />
          <textarea
            name="content"
            rows={2}
